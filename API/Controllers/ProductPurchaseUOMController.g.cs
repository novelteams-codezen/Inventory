...
{
    /// <summary>
    /// Controller responsible for managing productpurchaseuom-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ProductPurchaseUOMController : ControllerBase
    {
        ...
        public ProductPurchaseUOMController(inventoryContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new productpurchaseuom to the database</summary>
        ...
        {
            _context.ProductPurchaseUOM.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.ProductPurchaseUOM.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<ProductPurchaseUOM>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.ProductPurchaseUOM.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.ProductPurchaseUOM.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] ProductPurchaseUOM updatedEntity)
        {
            ...
            this._context.ProductPurchaseUOM.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ProductPurchaseUOM> updatedEntity)
        {
            ...
            var existingEntity = this._context.ProductPurchaseUOM.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ProductPurchaseUOM.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
