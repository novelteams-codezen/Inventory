...
{
    /// <summary>
    /// Controller responsible for managing stocktakeitembatches-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class StockTakeItemBatchesController : ControllerBase
    {
        ...
        public StockTakeItemBatchesController(inventoryContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new stocktakeitembatches to the database</summary>
        ...
        {
            _context.StockTakeItemBatches.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.StockTakeItemBatches.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<StockTakeItemBatches>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.StockTakeItemBatches.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.StockTakeItemBatches.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] StockTakeItemBatches updatedEntity)
        {
            ...
            this._context.StockTakeItemBatches.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<StockTakeItemBatches> updatedEntity)
        {
            ...
            var existingEntity = this._context.StockTakeItemBatches.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.StockTakeItemBatches.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
