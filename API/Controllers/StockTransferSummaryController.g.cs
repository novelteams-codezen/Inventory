...
{
    /// <summary>
    /// Controller responsible for managing stocktransfersummary-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class StockTransferSummaryController : ControllerBase
    {
        ...
        public StockTransferSummaryController(inventoryContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new stocktransfersummary to the database</summary>
        ...
        {
            _context.StockTransferSummary.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.StockTransferSummary.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<StockTransferSummary>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.StockTransferSummary.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.StockTransferSummary.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] StockTransferSummary updatedEntity)
        {
            ...
            this._context.StockTransferSummary.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<StockTransferSummary> updatedEntity)
        {
            ...
            var existingEntity = this._context.StockTransferSummary.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.StockTransferSummary.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
