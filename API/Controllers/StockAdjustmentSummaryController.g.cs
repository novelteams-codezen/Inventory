...
{
    /// <summary>
    /// Controller responsible for managing stockadjustmentsummary-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class StockAdjustmentSummaryController : ControllerBase
    {
        ...
        public StockAdjustmentSummaryController(inventoryContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new stockadjustmentsummary to the database</summary>
        ...
        {
            _context.StockAdjustmentSummary.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.StockAdjustmentSummary.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<StockAdjustmentSummary>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.StockAdjustmentSummary.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.StockAdjustmentSummary.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] StockAdjustmentSummary updatedEntity)
        {
            ...
            this._context.StockAdjustmentSummary.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<StockAdjustmentSummary> updatedEntity)
        {
            ...
            var existingEntity = this._context.StockAdjustmentSummary.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.StockAdjustmentSummary.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
