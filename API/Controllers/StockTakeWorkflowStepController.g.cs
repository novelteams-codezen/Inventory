...
{
    /// <summary>
    /// Controller responsible for managing stocktakeworkflowstep-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class StockTakeWorkflowStepController : ControllerBase
    {
        ...
        public StockTakeWorkflowStepController(inventoryContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new stocktakeworkflowstep to the database</summary>
        ...
        {
            _context.StockTakeWorkflowStep.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.StockTakeWorkflowStep.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<StockTakeWorkflowStep>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.StockTakeWorkflowStep.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.StockTakeWorkflowStep.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] StockTakeWorkflowStep updatedEntity)
        {
            ...
            this._context.StockTakeWorkflowStep.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<StockTakeWorkflowStep> updatedEntity)
        {
            ...
            var existingEntity = this._context.StockTakeWorkflowStep.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.StockTakeWorkflowStep.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
