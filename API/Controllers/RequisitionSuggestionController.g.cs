...
{
    /// <summary>
    /// Controller responsible for managing requisitionsuggestion-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class RequisitionSuggestionController : ControllerBase
    {
        ...
        public RequisitionSuggestionController(inventoryContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new requisitionsuggestion to the database</summary>
        ...
        {
            _context.RequisitionSuggestion.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.RequisitionSuggestion.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<RequisitionSuggestion>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.RequisitionSuggestion.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.RequisitionSuggestion.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] RequisitionSuggestion updatedEntity)
        {
            ...
            this._context.RequisitionSuggestion.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<RequisitionSuggestion> updatedEntity)
        {
            ...
            var existingEntity = this._context.RequisitionSuggestion.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.RequisitionSuggestion.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
