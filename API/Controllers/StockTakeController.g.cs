...
{
    /// <summary>
    /// Controller responsible for managing stocktake-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class StockTakeController : ControllerBase
    {
        ...
        public StockTakeController(inventoryContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new stocktake to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.StockTake.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<StockTake>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.StockTake.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.StockTake.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] StockTake updatedEntity)
        {
            ...
            this._context.StockTake.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<StockTake> updatedEntity)
        {
            ...
            var existingEntity = this._context.StockTake.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.StockTake.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
