...
{
    /// <summary>
    /// Controller responsible for managing productformulation-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ProductFormulationController : ControllerBase
    {
        ...
        public ProductFormulationController(inventoryContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new productformulation to the database</summary>
        ...
        {
            _context.ProductFormulation.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.ProductFormulation.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<ProductFormulation>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.ProductFormulation.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.ProductFormulation.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] ProductFormulation updatedEntity)
        {
            ...
            this._context.ProductFormulation.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ProductFormulation> updatedEntity)
        {
            ...
            var existingEntity = this._context.ProductFormulation.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ProductFormulation.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
