...
{
    /// <summary>
    /// Controller responsible for managing purchaseorderactivityhistory-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class PurchaseOrderActivityHistoryController : ControllerBase
    {
        ...
        public PurchaseOrderActivityHistoryController(inventoryContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new purchaseorderactivityhistory to the database</summary>
        ...
        {
            _context.PurchaseOrderActivityHistory.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.PurchaseOrderActivityHistory.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<PurchaseOrderActivityHistory>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.PurchaseOrderActivityHistory.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.PurchaseOrderActivityHistory.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] PurchaseOrderActivityHistory updatedEntity)
        {
            ...
            this._context.PurchaseOrderActivityHistory.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<PurchaseOrderActivityHistory> updatedEntity)
        {
            ...
            var existingEntity = this._context.PurchaseOrderActivityHistory.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.PurchaseOrderActivityHistory.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
