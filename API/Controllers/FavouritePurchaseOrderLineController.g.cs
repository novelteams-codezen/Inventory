...
{
    /// <summary>
    /// Controller responsible for managing favouritepurchaseorderline-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class FavouritePurchaseOrderLineController : ControllerBase
    {
        ...
        public FavouritePurchaseOrderLineController(inventoryContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new favouritepurchaseorderline to the database</summary>
        ...
        {
            _context.FavouritePurchaseOrderLine.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.FavouritePurchaseOrderLine.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<FavouritePurchaseOrderLine>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.FavouritePurchaseOrderLine.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.FavouritePurchaseOrderLine.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] FavouritePurchaseOrderLine updatedEntity)
        {
            ...
            this._context.FavouritePurchaseOrderLine.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<FavouritePurchaseOrderLine> updatedEntity)
        {
            ...
            var existingEntity = this._context.FavouritePurchaseOrderLine.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.FavouritePurchaseOrderLine.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
