...
{
    /// <summary>
    /// Controller responsible for managing producttheraputicclassification-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ProductTheraputicClassificationController : ControllerBase
    {
        ...
        public ProductTheraputicClassificationController(inventoryContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new producttheraputicclassification to the database</summary>
        ...
        {
            _context.ProductTheraputicClassification.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.ProductTheraputicClassification.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<ProductTheraputicClassification>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.ProductTheraputicClassification.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.ProductTheraputicClassification.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] ProductTheraputicClassification updatedEntity)
        {
            ...
            this._context.ProductTheraputicClassification.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ProductTheraputicClassification> updatedEntity)
        {
            ...
            var existingEntity = this._context.ProductTheraputicClassification.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ProductTheraputicClassification.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
