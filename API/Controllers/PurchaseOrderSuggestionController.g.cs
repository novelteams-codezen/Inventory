...
{
    /// <summary>
    /// Controller responsible for managing purchaseordersuggestion-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class PurchaseOrderSuggestionController : ControllerBase
    {
        ...
        public PurchaseOrderSuggestionController(inventoryContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new purchaseordersuggestion to the database</summary>
        ...
        {
            _context.PurchaseOrderSuggestion.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.PurchaseOrderSuggestion.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<PurchaseOrderSuggestion>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.PurchaseOrderSuggestion.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.PurchaseOrderSuggestion.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] PurchaseOrderSuggestion updatedEntity)
        {
            ...
            this._context.PurchaseOrderSuggestion.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<PurchaseOrderSuggestion> updatedEntity)
        {
            ...
            var existingEntity = this._context.PurchaseOrderSuggestion.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.PurchaseOrderSuggestion.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
