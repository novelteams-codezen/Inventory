...
{
    /// <summary>
    /// Controller responsible for managing stocktakeinitiated-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class StockTakeInitiatedController : ControllerBase
    {
        ...
        public StockTakeInitiatedController(inventoryContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new stocktakeinitiated to the database</summary>
        ...
        {
            _context.StockTakeInitiated.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.StockTakeInitiated.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<StockTakeInitiated>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.StockTakeInitiated.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.StockTakeInitiated.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] StockTakeInitiated updatedEntity)
        {
            ...
            this._context.StockTakeInitiated.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<StockTakeInitiated> updatedEntity)
        {
            ...
            var existingEntity = this._context.StockTakeInitiated.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.StockTakeInitiated.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
