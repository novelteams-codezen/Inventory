...
{
    /// <summary>
    /// Controller responsible for managing stockadjustmentworkflowstep-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class StockAdjustmentWorkFlowStepController : ControllerBase
    {
        ...
        public StockAdjustmentWorkFlowStepController(inventoryContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new stockadjustmentworkflowstep to the database</summary>
        ...
        {
            _context.StockAdjustmentWorkFlowStep.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.StockAdjustmentWorkFlowStep.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<StockAdjustmentWorkFlowStep>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.StockAdjustmentWorkFlowStep.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.StockAdjustmentWorkFlowStep.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] StockAdjustmentWorkFlowStep updatedEntity)
        {
            ...
            this._context.StockAdjustmentWorkFlowStep.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<StockAdjustmentWorkFlowStep> updatedEntity)
        {
            ...
            var existingEntity = this._context.StockAdjustmentWorkFlowStep.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.StockAdjustmentWorkFlowStep.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
